...
class Raw:
    def __init__(self, obs_files, freq_start=10, freq_end=80, start=None, end=None, block_start=0, block_end=1, verbose=False):
        ...
        for lane_idx, (fn_raw) in enumerate(self.fnames_raw):
            ...
            if (lane_idx == 0): #if first file
                ...
                if(self.subblocking < 8): self.subblocking=8
                self.block_start = int(np.floor(start/(self.time_int)))
                ...
                #    self.subblock_end = 1
                if(self.subblocking == 1): self.subblock_end = int(1)
    ...
    def __get_dedispers_block(self, block_num, file_num):
        ...
        if(self.overlap>0): tmp = tmp[:, :-self.overlap, :]

        blocksize_perchan = float(self.nschan_file/self.nof_polcpx)
        old_file_num = file_num-1
